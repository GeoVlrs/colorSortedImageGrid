...
import { timestampSlug, ensureUniquePath } from './output.js';
 ...
 *
 * `--animateValues` overrides the sensible default list above; sort-parameter
 * values are validated against the real key list so a typo fails fast with a
 * helpful message instead of silently producing an empty/identical frame.
 */
...
export async function runAnimation(options, logger) {
    ...
    // Named after what was swept, not a raw timestamp - "sortMethod-sweep" is
    // recognisable at a glance, an epoch number is not. An explicit
    // --animateOutput is written as given; the auto name gets the same
    // collision check as the grid's default filename.
    const target = options.animateOutput
        ? options.animateOutput
        : await ensureUniquePath(path.join('./output', `${options.animateOver}-sweep_${timestampSlug()}.gif`));
