...
// From constants.js rather than the modules that own the behaviour: sort.js,
// process.js and color.js pull in Jimp, culori, quantize and exifr, and none of
// that is needed to print help text or reject a bad flag.
import {
    SORT_METHODS, SORT_ORDERS, SORT_KEYS, parseSortKeys, VISUALIZATION_MODES, COLOR_METHODS
} from './constants.js';
import { checkGridArgs } from './validate.js';
// From constants.js, not the animator itself: that module pulls in Jimp,
// gifenc, culori and the whole render pipeline, and `--help` should not.
import { ANIMATE_DIMENSIONS, ANIMATE_MODES } from './animate/constants.js';
...
import { DEVELOPMENT_MODE_SEARCH_LIMIT } from './spotify/client.js';
import { DEFAULT_FPS } from './animate/constants.js';
...
function buildGridCommand(y) {
    return y
        ...
        .option('animate', { type: 'boolean', default: false, describe: 'Render an animated GIF (sweep mode unless --animateMode says otherwise)' })
        .option('animateMode', {
            type: 'string', choices: Object.values(ANIMATE_MODES),
            describe: 'sweep: cut between settings. build: reveal the grid one tile at a time. morph: tiles fly from unsorted into sorted order. Implies --animate'
        })
        ...
        .option('animateFps', { type: 'number', default: DEFAULT_FPS, describe: 'Frames per second. GIF stores delays in 10ms steps, so values are snapped' })
        .option('animateHoldMs', { type: 'number', default: 2000, describe: 'How long the finished result is held at the end' })
        .option('animateEasing', { type: 'string', choices: EASING_NAMES, describe: 'Motion curve for tweened modes' })
        .group(['animate', 'animateMode', 'animateWidth', 'animateOutput', 'animateFps', 'animateHoldMs', 'animateEasing'], 'Animation:')

        .option('animateOver', {
            type: 'string', choices: Object.values(ANIMATE_DIMENSIONS), default: ANIMATE_DIMENSIONS.SORT_PARAMETER,
            describe: 'Which setting varies between frames (sweep only)'
        })
        .option('animateValues', { type: 'string', describe: 'Comma-separated values to step through (sweep only; defaults to all valid ones)' })
        .option('animateDelay', { type: 'number', default: 600, describe: 'Milliseconds per frame (sweep only; other modes use --animateFps)' })
        .option('revealPerFrame', { type: 'number', describe: 'Tiles revealed per frame (build only; defaults to whatever keeps it under 240 frames)' })
        .option('morphSeconds', { type: 'number', default: 3, describe: 'How long the tween lasts (morph only)' })
        .option('morphStagger', { type: 'number', default: 0.3, describe: 'How much tiles trail each other, 0-0.9 (morph only). 0 moves them all together' })
        .option('morphHoldMs', { type: 'number', default: 700, describe: 'Pause on the unsorted layout before moving (morph only)' })
        .group(['animateOver', 'animateValues', 'animateDelay', 'revealPerFrame', 'morphSeconds', 'morphStagger', 'morphHoldMs'], 'Animation, per mode:')
        ...
        .example('$0 --animateMode build', 'GIF of the grid assembling one tile at a time')
        .example('$0 --animateMode morph', 'GIF of tiles flying from filename order into colour order')
        ...
        .check((args) => {
            ...
            // Every numeric range rule lives in validate.js, so the interactive
            // wizard can reject the same values at the prompt rather than
            // re-implementing a subset of them.
            checkGridArgs(args);

            const animating = args.animate || args.animateMode !== undefined;

            // index.js dispatches to the animator before it looks at --watch, so
            // asking for both used to silently drop the watch. Say so instead.
            if (animating && args.watch) {
                throw new Error('--watch cannot be combined with an animation; pick one.');
            }

            // Flags that only mean something in one mode. yargs' strict mode
            // catches unknown flags but not ones that are merely irrelevant
            // here, and silently ignoring them is how people end up puzzled
            // that their setting had no effect.
            const mode = args.animateMode ?? ANIMATE_MODES.SWEEP;
            if (mode !== ANIMATE_MODES.SWEEP) {
                for (const [flag, hint] of [
                    ['animateDelay', 'use --animateFps'],
                    ['animateOver', 'that is a sweep setting'],
                    ['animateValues', 'that is a sweep setting']
                ]) {
                    // Compare against the parsed default, since yargs fills it in.
                    if (args[flag] !== undefined && process.argv.includes(`--${flag}`)) {
                        throw new Error(`--${flag} only applies to --animateMode sweep (${hint}).`);
                    }
                }
            }
            return true;
...
function buildFetchCommand(y) {
    return y
        ...
        .option('searchLimit', {
            type: 'number', default: DEVELOPMENT_MODE_SEARCH_LIMIT,
            describe: 'Candidates to consider per album. Spotify Development Mode apps reject values ' +
                `above ${DEVELOPMENT_MODE_SEARCH_LIMIT} ("Invalid limit") - a higher value is retried at ` +
                'that limit automatically if rejected, so it is safe to raise this if your app has extended access'
        })
...
export function resolveOptions(args) {
    ...
    return {
        ...
        // Naming a mode implies wanting an animation, so `--animateMode build`
        // works without also passing `--animate`. Bare `--animate` still means
        // sweep, which keeps the documented invocation working.
        animate: args.animate || args.animateMode !== undefined,
        animateMode: args.animateMode ?? ANIMATE_MODES.SWEEP,
        ...
        animateFps: args.animateFps,
        animateHoldMs: args.animateHoldMs,
        animateEasing: args.animateEasing,
        revealPerFrame: args.revealPerFrame,
        morphSeconds: args.morphSeconds,
        morphStagger: args.morphStagger,
        morphHoldMs: args.morphHoldMs,
