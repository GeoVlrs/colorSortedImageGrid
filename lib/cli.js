...
function buildGridCommand(y) {
    return y
        ...
        .check((args) => {
            ...
            for (const key of ['numRows', 'numColumns', 'pxPerImage', 'sortBands', 'concurrency', 'sampleSize',
                'animateWidth', 'animateFps', 'revealPerFrame', 'morphSeconds']) {
            ...
            if (args.morphStagger < 0 || args.morphStagger > 0.9) {
                throw new Error('--morphStagger must be between 0 and 0.9.');
            }
            if (args.animateHoldMs < 0 || args.morphHoldMs < 0) {
                throw new Error('--animateHoldMs and --morphHoldMs cannot be negative.');
            }
