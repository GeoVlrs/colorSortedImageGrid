...
/**
 * Put chosen section ids back into `SECTIONS` order, dropping anything unknown.
 *
 * Needed because a checkbox list hands back its values in the order the boxes
 * were *checked*, not the order they were listed. Sections are not independent:
 * entering the animation section sets `animate`, and the run section reads that
 * to decide whether `watch` is even offered. Checking "Run mode" before
 * "Animation" would therefore offer a watch alongside an animation, which the
 * parser rejects outright.
 *
 * @param {string[]} ids
 * @returns {string[]}
 */
export function orderSections(ids) {
