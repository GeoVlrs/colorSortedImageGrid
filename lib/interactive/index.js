...
/**
 * What each sort key actually means, for every select/multiselect that lists
 * them. Without this, someone who doesn't already know the difference between
 * `value` and `lightness` (both "brightness", one HSV and one HSL - see
 * lib/color.js) is picking blind.
 *
 * `label` is only set where the raw key reads oddly as a prompt option -
 * `dateTaken` is camelCase because it is a flag name, not a sentence.
 */
const SORT_KEY_INFO = {
...
/** `{value, label, hint}` list for a set of sort keys, using `SORT_KEY_INFO`. */
function sortKeyOptions(keys) {
    return keys.map((key) => ({
        value: key,
        label: SORT_KEY_INFO[key]?.label ?? key,
        hint: SORT_KEY_INFO[key]?.hint
    }));
}
...
export const PROMPTS = {
    ...
    sortSecondary: {
        ...
        options: () => sortKeyOptions(SORT_KEYS)
    },
...
async function ask(option, current, answers) {
    ...
    if (spec.type === 'tiebreaks') {
        ...
        const chosen = guard(await clack.multiselect({
            ...
            options: sortKeyOptions(SORT_KEYS.filter((key) => key !== primary))
        }));
...
async function runSpine(answers) {
    ...
    if (answers.sortMethod === SORT_METHODS.NUMERIC || answers.sortMethod === SORT_METHODS.BANDED) {
        const chosen = guard(await clack.select({
            ...
            options: sortKeyOptions(SORT_KEYS.filter((key) => key !== 'filename'))
        }));
...
export async function runInteractive(baseOptions) {
    ...
    // Previously a checkbox multiselect: press Space to check an option, Enter
    // to submit whatever is checked. In practice that Space step turned out to
    // be unreliable enough (terminal-dependent - arrowing to an option and
    // pressing Enter does not check it, and the difference is easy to miss)
    // that people kept landing on an empty selection without meaning to, which
    // - since the list is optional - looked identical to "any choice here
    // skips straight to render". A plain yes/no per section needs nothing but
    // Enter, matching every other prompt in the wizard, and one gate up front
    // keeps declining everything down to a single Enter, same as before.
    const sections = [];
    ...
    if (wantsMore) {
        for (const section of SECTIONS) {
            ...
            if (wants) sections.push(section.id);
        }

        if (sections.length === 0) {
            clack.log.info('Nothing chosen - everything else stays at its default.');
        }
    }
