...
export async function runInteractive(baseOptions) {
    ...
    // clack's multiselect submits on Enter whatever is currently *checked*
    // with Space - arrowing onto an item and pressing Enter does not check it,
    // it just moves the cursor there. Because this list is optional
    // (`required: false`), that mistake produces an empty selection with no
    // error, which looks exactly like "picking anything jumps straight to
    // render". Spelling out the keys here, rather than relying on clack's
    // default footer hint, is what fixes that.
    const sections = guard(await clack.multiselect({
        message: 'Configure anything else? (space to check a box, enter to confirm; leave all unchecked to skip)',
        required: false,
        initialValues: [],
    ...
    if (sections.length === 0) {
        clack.log.info('Nothing checked - everything else stays at its default.');
    }
