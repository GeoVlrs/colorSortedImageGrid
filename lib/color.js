...
import { COLOR_METHODS } from './constants.js';

// Defined in lib/constants.js so the CLI can read it without loading Jimp,
// culori, quantize and exifr just to list two option values.
...
/**
 * RGB to a `#rrggbb` string, one channel at a time.
 *
 * Not how the original derived its hex string (see extractAverageColor below):
 * padding each channel independently means a channel below 16 can never shift
 * the others, which is exactly the bug this replaced.
 */
